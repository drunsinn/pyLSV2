...
class ToolTable():
    ...
    def __init__(self, table_path):
        ...
        with table_file.open(mode='r') as tfp:
            ...
            header = re.match(r"^BEGIN ([A-Z]*)\.([A-Z]{1,3}) (MM|INCH)(?: Version: \'Update:(\d+\.\d+)\')?$", header_line)
            ...
            logging.debug('Header Infomation for file %s Name %s, file is metric %s, Version %s', table_file, self._name, self._is_metric, self._version)

            column_header = tfp.readline()

            column_list = list()
            column_pattern = re.compile(r"([A-Z-12\.]+)(?:\s+)")
            for m in column_pattern.finditer(column_header):
                column_list.append({'start': m.start(), 'end': m.end()-1, 'name': m.group().strip()})
