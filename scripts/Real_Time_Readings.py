...
import logging
import pyLSV2
from pyLSV2.const import channel_signal
...
with pyLSV2.LSV2("192.168.56.103", port=19000, timeout=5, safe_mode=False) as con:
    ...
    selected_signals.append(availible_signals[channel_signal.s_actual_X])
    selected_signals.append(availible_signals[channel_signal.s_actual_Y])
    selected_signals.append(availible_signals[channel_signal.s_actual_Z])
    selected_signals.append(availible_signals[channel_signal.v_actual_X])
    selected_signals.append(availible_signals[channel_signal.a_actual_X])
