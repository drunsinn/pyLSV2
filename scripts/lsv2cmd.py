...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--force", help="replace file at target if it already exists",
    ...
    if dest_match is not None:
        ...
        if dest_match.group("prot") == 'ssh':
            use_ssh = True
        logging.info('Destination path %s is on remote %s:%s via %s',
    ...
    if use_ssh:
        import sshtunnel
        ssh_forwarder = sshtunnel.SSHTunnelForwarder(host_machine,
    ...
    if source_is_remote:
        ...
        if not file_info:
            ...
            logging.error('source on remote is not file but directory: "%s"', source_path)
            sys.exit(-4)
    else:
    ...
    if source_is_remote and dest_is_remote:
        ...
        success = con.copy_local_file(source_path=source_path,
                                      target_path=dest_path)
    elif source_is_remote and not dest_is_remote:
        logging.debug("copy from remote to this device")
        success = con.recive_file(remote_path=source_path,
                                  local_path=dest_path,
                                  override_file=args.force)
    else:
        logging.debug("copy from this device to remote")
        success = con.send_file(local_path=source_path,
