...
def test_login(address, timeout):
    ...
    lsv2.connect()
    assert lsv2.login(login=pyLSV2.LOGIN_INSPECT) is True
    assert lsv2.login(login=pyLSV2.LOGIN_FILETRANSFER) is True
    assert lsv2.login(login=pyLSV2.LOGIN_DNC) is False
    lsv2.disconnect()
