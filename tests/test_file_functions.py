...
def test_folder_functions(address):
    ...
    assert lsv2.delete_empty_directory(
        'TNC:/nc_prog/pyLSV2_test/T1/T2/T3') == True
    ...
    assert lsv2.delete_empty_directory(
        'TNC:/nc_prog/pyLSV2_test/T1/T2') == False
    assert lsv2.change_directory('TNC:/nc_prog') == True
    assert lsv2.delete_empty_directory(
        'TNC:/nc_prog/pyLSV2_test/T1/T2') == True
...
def test_remote_file_functions(address):
    ...
    assert lsv2.copy_local_file(
        source_path='TNC:/nc_prog/$mdi.h', target_path='TNC:/nc_prog/pyLSV2_test/') == True
    assert lsv2.get_file_info('TNC:/nc_prog/pyLSV2_test/$mdi.h') != False

    assert lsv2.copy_local_file(
        source_path='TNC:/nc_prog/$mdi.h', target_path='TNC:/nc_prog/pyLSV2_test/aa.h') == True
    assert lsv2.get_file_info('TNC:/nc_prog/pyLSV2_test/aa.h') != False

    assert lsv2.copy_local_file(
        source_path='TNC:/nc_prog/$mdi.h', target_path='mdi.b') == True
    assert lsv2.get_file_info('TNC:/nc_prog/mdi.b') != False

    assert lsv2.change_directory(remote_directory='TNC:/nc_prog') == True
    assert lsv2.copy_local_file(
        source_path='$mdi.h', target_path='TNC:/nc_prog/pyLSV2_test/bb.h') == True
    ...
    assert lsv2.move_local_file(source_path='TNC:/nc_prog/pyLSV2_test/bb.h',
                                target_path='TNC:/nc_prog/pyLSV2_test/cc.h') == True
